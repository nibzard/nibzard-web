...
Always keep `LESSONS_LEARNED.md` updated whenever we overcome an obstacle or learn something new about maintaining this website or writing articles. This creates a growing knowledge base of solutions and insights.
...
## Git Workflow
- Before pushing to git always test the build with: `pnpm run build`
...
- **now-post-writer**: Same destination and format as now entries, optimized for periodic status updates (`src/content/now/`)
...
| now-post-writer | `src/content/now/` | YYMMDD.md | Minimal (date only), current-focus updates |
...
This site supports Mermaid diagrams in markdown files. Mermaid content is transformed at build time and rendered in-browser.

### Setup and Dependencies
- **Required packages**: none for Mermaid parsing itself
- **Optional packages**: `puppeteer` is used for OG image generation; `sharp` is optional for image optimization
- **Browser runtime for OG generation (if you need fresh OG images)**: `pnpm exec puppeteer browsers install chrome`
- **Configuration**: Already configured in `astro.config.mjs` and `src/plugins/remark-mermaid-client.mjs` with:
  - `excludeLangs: ['mermaid']` to prevent syntax highlighting interference
  - a custom remark plugin that converts Mermaid code blocks into `<div class="mermaid">...</div>`
...
### Troubleshooting
- If Mermaid diagrams show as code blocks, verify `/scripts/mermaid-loader.js` is included in the page pipeline.
- If content is truncated after Mermaid blocks, check raw markdown fences and Mermaid syntax before plugin parsing.
- Mermaid blocks are transformed during Markdown processing and rendered client-side by the site loader.
