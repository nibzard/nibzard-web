...
The external camp has been working hard on the chattiness, which is exactly what those Playwright-to-raw-CDP moves are about: killing a relay hop. But moving the driver closer to the wire doesn't change *which side* the driver is on.
...
The part worth remembering is the inversion, not the extension or the Steel trick. Stop driving the browser from outside. Move the loop inside the session, demote CDP from control to retrieval, and let the browser be the environment the agent lives in, not the thing it's poking with a stick.
